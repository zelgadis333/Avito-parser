...
def load_user_data(page, session):
    url = 'https://www.avito.ru/moskva/noutbuki?p=%d' % (page)
    # print url
...
def parse_user_datafile_bs(filename):
    ...
    for item in item_list:
        ...
        try:
            ...
            item_date_add = ""
        try:
            item_adress = soup.find_all('div', {'class': 'seller-info-prop'})[4].find('div', {'class': 'seller-info-value'}).text
        except:
            item_adress = ""
        try:
            ...
            item_owner = ""
            resultsanswer.append([item_link, item_title, item_price, item_date_add, avito_id, item_description, item_adress, item_owner])
...
for i in range(0, len(results)):
    writer.writerow({'item_link': results[i][0], 'item_title': results[i][1], 'item_price': results[i][2], 'avito_id': results[i][4]})
f.close()
